...
PROBLEMS:
----------
ADDITIONAL FEATURES:When up arrow is pressed, ufo gets fasterin to up direction. when down arrow is pressed ufo moves down.
      Moving around states by using up and down arrow.
	    Displaying the coordinates and messages when each button is clicked
	  Each states have their own background and shapes.
*********/
...
void movedown()
{
	if (animating && ufo_y > -90)
	{
		ufo_y -= 60;
			...
			if (appState == SPACE && ufo_y < -125)
			{
				appState = RUN;
				ufo_y = 135;
			}

			else if (appState == OUTOFSPACE && ufo_y < -100)
			{
				appState = SPACE;
				ufo_y = 105;
...
void display_outofspace()
{
	display_start();
	//star

	//48, 25, 52
	glColor3f(48/255.0, 25/255.0, 52/255.0);
...
void display() {
	...
	case SPACE:display_space();
		break;
	case OUTOFSPACE:display_outofspace();
		break;
...
void onTimer(int v) {
	if (animating) {
		...
		// Write your codes here.
		if (appState == RUN || appState == SPACE||appState==OUTOFSPACE) {
			t += 1;
			...
			if (ufo_y > -155 && ufo_y < 200)
				ufo_y += 15;
			else if (ufo_y > 200&&appState==RUN)
			{
				ufo_y = -150;
				appState = SPACE;
			}
			else if (ufo_y > 200 && appState == OUTOFSPACE)
			{
